...
def get_glider_transect_from_GOFS(url_model,var_name_model,model_name,varg,timeg,latg,long,depthg,contour_plot='yes'):
    ...
    tstamp_model = [mdates.date2num(time_model[i]) for i in np.arange(len(time_model))]
    ...
    cs = plt.contourf(mdates.date2num(time_model),-depth_model,var_model,cmap=color_map,**kw)
    plt.contour(mdates.date2num(time_model),-depth_model,var_model,[26],colors='k')
    ...
    return var_model, time_model, depth_model, lat_model, lon_model
