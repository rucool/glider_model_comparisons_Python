...
#%%
def get_transect_from_GOFS(url_model,var_name_model,model_name,time,lat,lon,contour_plot='yes'):
    ...
    This function finds any transect in
    the Global Ocean Forecasting System (GOFS 3.1)
    given time, latitude and longitude
    ...
    # Changing times to timestamp
    tstamp_glider = [mdates.date2num(time[i]) for i in np.arange(len(time))]
    ttmodel = np.asarray([datetime(time_model[i].year,time_model[i].month,time_model[i].day,\
                    time_model[i].hour) for i in np.arange(len(time_model))])
